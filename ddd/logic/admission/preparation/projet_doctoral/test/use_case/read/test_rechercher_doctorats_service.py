...
class TestRechercherDoctoratService(SimpleTestCase):
    ...
    def test_should_renvoyer_detail_doctorat(self, mock_search_formations):
        mock_search_formations.return_value = [_get_training_dto('SST', 'AGRO3DP', 2020)]
        result = self.message_bus.invoke(self.cmd)
        dto = result[0]
        self.assertEqual(dto.sigle, 'AGRO3DP')
        self.assertEqual(dto.annee, 2020)
        self.assertEqual(dto.sigle_entite_gestion, 'SST')
        self.assertEqual(dto.intitule_fr, 'Intitule du doctorat')
        self.assertEqual(dto.intitule_en, 'Title of PHD')
