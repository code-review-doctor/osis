...
class CohortesDuGestionnaireInMemory(ICohortesDuGestionnaire):

    cohortes_gestionnaires = {
        CohorteGestionnaireDTO(
            matricule_gestionnaire='22220000',
            nom_cohorte='DROI1BA',
        ),
    }
    ...
    def search(
            ...
            anneee_concernee: int,
    ) -> Set['CohorteGestionnaireDTO']:
        return set(
            filter(
                lambda dto: dto.matricule_gestionnaire == matricule_gestionnaire,
                cls.cohortes_gestionnaires,
            )
        )
