...
class ScoreSheetXLSExportAPIViewTestCase(APITestCase):
    @classmethod
    def setUpTestData(cls):
        cls.tutor = TutorFactory()
        cls.url = reverse('assessments_api_v1:' + ScoreSheetXLSExportAPIView.name, kwargs={
            'learning_unit_code': 'LDROI1200'
        })
    ...
    def setUp(self):
        ...
        self.patch_message_bus = mock.patch(
            "assessments.api.views.score_sheet_xls_export.message_bus_instance.invoke",
    ...
    def test_get_assert_return_xls_response(self):
        response = self.client.get(self.url)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn(CONTENT_TYPE_XLS, response['content-type'])
