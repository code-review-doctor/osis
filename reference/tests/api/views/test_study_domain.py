...
class StudyDomainListTestCase(APITestCase):
    @classmethod
    def setUpTestData(cls):
        cls.user = UserFactory()
        cls.url = reverse('reference_api_v1:' + StudyDomainList.name)

        DomainFactory(adhoc=False, parent=None)
        DomainFactory(adhoc=False, parent=None)
    ...
    def test_get_ensure_response_have_next_previous_results_count(self):
        ...
        expected_count = Domain.objects.all().count()
    ...
    def test_get_ensure_default_order(self):
        ...
        study_domain_qs = Domain.objects.filter(parent__isnull=True, adhoc=False).order_by('name')
        serializer = StudyDomainSerializer(study_domain_qs, many=True)
