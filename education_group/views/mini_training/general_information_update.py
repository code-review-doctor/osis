...
class MiniTrainingUpdateGeneralInformation(MiniTrainingRead, UpdateCmsView):
    ...
    def get_reference(self):
        return self.get_education_group_version().offer_id

    def get_references(self):
        return EducationGroupYear.objects.filter(
            education_group=self.get_education_group_version().offer.education_group,
            academic_year__year__gte=self.get_education_group_version().offer.academic_year.year
        ).values_list(
            'id',
            flat=True
        )
    ...
    def can_postpone(self) -> bool:
        return can_postpone_general_information(self.get_education_group_version().offer)
