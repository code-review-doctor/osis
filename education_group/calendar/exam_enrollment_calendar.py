...
class ExamEnrollmentSubmissionCalendar(AcademicEventSessionCalendarHelper):
    event_reference = academic_calendar_type.EXAM_ENROLLMENTS
    ...
    def _create_session_one(cls, academic_year: AcademicYear):
        ...
        if not AcademicCalendar.objects.filter(**kwargs).exists():
            academic_calendar = AcademicCalendar.objects.create(
                ...
                start_date=datetime.date(academic_year.year, 11, 1),
                end_date=datetime.date(academic_year.year, 11, 30),
            )
    ...
    def _create_session_two(cls, academic_year: AcademicYear):
        ...
        if not AcademicCalendar.objects.filter(**kwargs).exists():
            academic_calendar = AcademicCalendar.objects.create(
                ...
                start_date=datetime.date(academic_year.year + 1, 3, 1),
                end_date=datetime.date(academic_year.year + 1, 4, 30),
            )
