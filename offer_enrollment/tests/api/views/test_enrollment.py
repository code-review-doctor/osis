...
class OfferEnrollmentsListViewTestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.url = reverse(
            ...
            kwargs={'registration_id': cls.student.registration_id}
        )
    ...
    def test_get_results_assert_acronym_11ba_is_correct(self):
        ...
        url = reverse(
            'offer_enrollment_api_v1:' + OfferEnrollmentsListView.name,
            kwargs={'registration_id': offer_enrollment_11ba.student.registration_id}
        )
        self.client.force_authenticate(user=offer_enrollment_11ba.student.person.user)
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_no_result_for_other_registration_id(self):
        url = reverse(
            'offer_enrollment_api_v1:' + OfferEnrollmentsListView.name,
            kwargs={'registration_id': self.student.registration_id + '2'}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
