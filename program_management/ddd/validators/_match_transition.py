...
class MatchTransitionValidator(business_validator.BusinessValidator):
    ...
    def __init__(
        ...
        super(MatchTransitionValidator, self).__init__()
    ...
    def validate(self):
        try:
            child_transition_name = self.get_node_to_add_version()
        ...
        transition_mismatched = [
            parent_version.entity_id for parent_version in parent_versions
            if parent_version.transition_name != child_transition_name
        ]
        if transition_mismatched:
    ...
    def get_node_to_add_version(self) -> str:
        if self.node_to_add.is_training():
            return self.node_to_add.transition_name or ''

        return next(node.transition_name for node in self.node_to_add.get_training_children())
