...
class TestMatchTransitionValidator(TestValidatorValidateMixin, SimpleTestCase):
    def setUp(self):
        tree_data = {
    ...
    def test_should_not_raise_exception_when_transition_match(self):
    ...
    def test_should_raise_exception_when_versions_mismatch(self):
        tree_to_attach = tree_builder(
            {"node_type": TrainingType.MASTER_MA_120, "transition_name": "SPECIFIC", "end_year": 2019}
        )
        tree_to_attach_version = StandardTransitionProgramTreeVersionFactory(tree=tree_to_attach)
        ...
        tree_to_attach = tree_builder(
            {
                "node_type": GroupType.FINALITY_120_LIST_CHOICE,
                "end_year": 2019,
                "children": [{"node_type": TrainingType.MASTER_MA_120, "transition_name": "SPECIFIC", "end_year": 2019}]
            }
        )
        tree_to_attach_version = StandardTransitionProgramTreeVersionFactory(tree=tree_to_attach)
    ...
    def test_should_not_raise_exception_when_minitraining_version_mismatch(self):
        tree_to_attach = tree_builder(
            {"node_type": MiniTrainingType.OPEN_MINOR, "transition_name": "SPECIFIC", "end_year": 2019}
        )
        tree_to_attach_version = StandardTransitionProgramTreeVersionFactory(tree=tree_to_attach)
